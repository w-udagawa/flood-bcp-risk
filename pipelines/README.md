...
## Fable による判断（2026-09-09、詳細は `docs/06_設計判断ログ.md`）

- J-09：P04/P14/N02 のフラグは usage_code より優先する（採用）。
- J-10：datacenter は管理施設登録（FR-08）で usage_class を上書きして扱う。
- J-11：alt_facility_dist_m は M2 で commercial と commercial_large を同一視するよう変更する。
- J-14：`30_terrain.py` の距離計算は EPSG:6677 に投影して行うよう M2 で修正する（TODO）。
- J-15：hazard_source_year は最も古いソースの年を入れる。
