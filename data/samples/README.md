...
接頭辞を付けた仮名です。**名称は全て架空であり、実在の駅名・地名・区名（自由が丘・
目黒・世田谷・大田・渋谷・等々力 等）は一切含みません**（「架空:A駅ビル」
「架空:B地区中央病院」の「A」「B」「C」は建物どうしの地域的なまとまりを表す
ための架空の記号で、特定の実在区・地名を指すものではありません）。
ward_code の列だけは目黒区(13110)・世田谷区(13112)・大田区(13113)の実在コードを
地域感を出すために借用していますが、建物そのものは実在せず、名称にはこれらの
区名を使用していません。
...
- シナリオ：
  - `bldg_sample_001`：低地立地の大規模商業施設型（commercial_large、
    storeys_below=1、内水 0.5–1.0 m、延床 ≥10,000、浸水実績あり）
    → 優先度 A を想定
